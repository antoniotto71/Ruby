...
valid = passports.select {|passport| (required_fields - passport.keys) == []}

puts valid.count

strict_valid = valid.select do |p|
  # a = (1920..2002).cover?(p["byr"].to_i)
  # b = (2010..2020).cover?(p["iyr"].to_i)
  # c = (2020...2030).cover?(p["eyr"].to_i)
  # d = p["hcl"] =~ /^#\h{6}$/
  # e = %w(amb blu brn gry grn hzl oth).include?(p["ecl"])
  # f = p["pid"] =~ /^\d{9}$/
  # g = p["hgt"] =~ /cm$/ && (150..193).cover?(p["hgt"].to_i)
  # h = p["hgt"] =~ /in$/ && (59..76).cover?(p["hgt"].to_i)
  # [a, b, c, d, e, f, (g || h)].all?
